<template>
  <div class='container'>
    <div class='yuyue_nav'>
      <div class='yuyue_nav_item' :class="tabcheck == 0?'checked':'nochecked'" @tap='checkTab(0)'>
        品牌公寓
      </div>
      <div class='yuyue_nav_item' :class="tabcheck == 1?'checked':'nochecked'" @tap='checkTab(1)'>
        整租/合租
      </div>
    </div>
    <div class='content-box' v-if='tabcheck == 0'>
      ...
      <div class='yuyue_list' v-if='roomList.length>0'>
        <div v-for='(fangyuanItem, index) in roomList' :key="index">
          <apart-ment :fangyuanItem=fangyuanItem @getLoad="getOrder"></apart-ment>
        </div>
      </div>
    ...
    <div class='content-box' v-if='tabcheck === 1'>
      ...
      <div class='yuyue_list' v-if='houseList.length>0'>
        <div v-for='(houseItem, index) in houseList' :key="index">
          ...
          <all-together :fangyuanItem=houseItem @geLoad="getOrder"></all-together>
        </div>
      </div>
      <div class='yuyue_list no_content' v-else>没有任何内容！</div>
    </div>
...
import apartMent from '../template/apartment';
...
export default {
  ...
  methods: {
    ...
    getOrder: function() {
      this.$store
        .dispatch('getNum', 'orderNum')
        .then(res => {
          if (res.status.code === '200') {
            const gongyuList = [];
            const zhengzuList = [];
            const houseArr = res.result.renterInfoArr;
