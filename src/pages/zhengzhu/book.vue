<template>
  <div class='container'>
    ...
    <div class='submit' @tap='bookNow'>{{editorText}}</div>
  </div>

</template>

<script>
import { showToast, checkMobile, getItem } from '@/utils';
export default {
  name: 'book',
  props: {
    bookType: {
  ...
  methods: {
    getOrderHouse() {
      ...
      this.$store.dispatch('getOrderHouse', params).then(res => {
    ...
    bookNow() {
      ...
      if (!getItem('phone')) {
        showToast({ title: '请先登录下哦！' })
        return
      }
      if (this.bookType === 'book') {
        const params = {
          id: this.id,
          isJizhong: 0,
          name: this.userName,
          phone: this.userPhone,
          needRemark: this.userBeizhu,
          seeTime: this.time
        }
        this.$store.dispatch('bookHouse', params)
          .then(res => {
            if (res.status.code === '200') {
              showToast({ title: '预约成功' })
              const that = this
              setTimeout(() => {
                that.$router.push({
                  path: '/pages/my/index',
        ...
        const params = {
          houseId: this.id,
          isJizhong: 0,
          zukeName: this.userName,
          zukePhone: this.userPhone,
          endtime: this.time
        }
        this.$store.dispatch('bookPay', params)
          .then(res => {
            console.log(res)
            if (res.status.code === '200') {
              // 支付
